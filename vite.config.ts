...
/**
 * Khởi tạo kết nối MySQL trực tiếp với 1 Persistent Connection duy nhất và Keep-Alive liên tục
 * để chống vượt quá hạn ngạch 500 kết nối/giờ (max_connections_per_hour) của Hostinger.
 */
...
async function executeDbWithLimit<T>(fn: (db: mysql.Pool) => Promise<T>, fallbackData?: any): Promise<T> {
  ...
  try {
    const db = getDbPool();
    // Bọc timeout 5 giây cho mỗi truy vấn
    ...
    if (errorInfo.reason === 'quota_reached') {
      // Khi vượt quota, tạm ngắt trong 30 giây để tránh spam
