...
export const storageService = {
  ...
  getSystemSettings(): SystemSettings { 
    const settings = this.getData(DB_KEYS.SYSTEM_SETTINGS, DEFAULT_SETTINGS); 
    if (!settings.banners || settings.banners.length === 0) {
      settings.banners = DEFAULT_SETTINGS.banners;
    }
    return settings;
  },
